...
def query():
    ...
    try:
        for doc in cursor:
            data.append([doc['reply_or_quote'].lower(), doc['coherencia']])
    finally:
        client.close()
    df = pd.DataFrame(data,columns=['tweet', 'coherencia'])
    return df


df = query()

df['coherencia'] = df['coherencia'].astype('int')
df['tweet']=df['tweet'].apply(lambda x: quitar_cuentas(x))

## Train y test para el modelo
X_train, X_test, y_train, y_test = train_test_split(df['tweet'], df['coherencia'], random_state = 0)
...
dump(clf, cwd + '/assets/pys/modelo_coherencia_col.joblib') 

import pickle
pickle.dump(count_vect.vocabulary_,open( cwd + "/assets/pys/vocabulario_coherencia_col.pkl","wb"))