...
def retrieve_initial_tweets(screen_name):
    ...
    for tweet in tweepy.Cursor(api.search, q='{}'.format(screen_name), lang="es", tweet_mode='extended').items(50):
...
def main(pais):
    ...
    if pais=='ARG':
        cuenta=cuentas_arg
    for i in cuenta:
        tweets = tweepy.Cursor(api.user_timeline, screen_name=i, tweet_mode='extended').items(5)
